jQuery(document).ready(function($){
  var CONSTANTS = {} 
    , social = { fb: {}, twttr: {} }
    , options = {}
    , ballotUrl = 'data/ballot' 
    , coachesUrl = 'data/coaches'
  ...
  CONSTANTS['ASSET_PATH'] = espn.core.cdnHTTPPath + 'contests/infiniti/2014/coaches-charity-challenge/';
  CONSTANTS['THUMBNAIL_PATH'] = CONSTANTS['ASSET_PATH'] + 'thumbnails/';
  ...
  */
  ...
  function initThanksSharing(){
    ...
    overrides = {
      ...
      picture: CONSTANTS['THUMBNAIL_PATH'] + coach.thumbnail.base,
  ...
  function getCurrentCoach(){
    ...
    if (!(window.coaches && params.coachId && parseInt(params.coachId,10))){
      return coach;
    }
    
    coachId = parseInt(params.coachId,10);
  ...
  function renderCoaches(selector, data){
    ...
    if ($('body').hasClass('info')){
      ...
      data.sort(function(a,b){
        return a.name.sort.localeCompare(b.name.sort);
      });
    }
    ...
    $.each(data, function(i, item){
      ...
      var $itemContainer = $(selector).first().clone(true, true)
        , ranking = item.rank
        , pct = Math.round((item.votes / (window.ballot.totalVotes * 1.0) * 100), 10)
        , params = { next: 'vote', coachId: item._id, rel: Modernizr.touch ? 'next' : 'modal' }
        , href = $.param(params)
      ;
      ...
      $itemContainer.find('.display-name-school').text(item.school.name.full);
      $itemContainer.find('.display-name-charity').text(item.charity.name.full);
      $itemContainer.find('.sort-name-coach').text(item.name.sort);
      $itemContainer.find('.sort-name-school').text(item.school.name.sort);
      $itemContainer.find('.sort-name-charity').text(item.charity.name.sort);
      $itemContainer.find('.display-result').text(pct + '%');
      $itemContainer.find('.sort-result').text(pct / 100.0);
      $itemContainer.find('[data-control-action="submit-vote"]').attr('href', '?' + href).attr('rel', params.rel);
  ...
  function loadCoaches(){
    var settings = {
      ...
      cache: false,
      success: function(data){
        return $(document).trigger({ type:'coachesloaded', coaches: data });
      },
      error: function(jqXhr, textStatus, errorThrown){
        return debug.error('Error loading ballot [' + textStatus + ']: ' + errorThrown);
      }
